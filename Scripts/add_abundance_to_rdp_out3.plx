#!/usr/bin/perl
#April 4, 2017 by Teresita M. Porter
#Script to add read abundance from cat.table to RDP taxonomic assignments
...
while (($global_otu, $assignment) = each %assignment) {
        ...
        if (exists $table{$global_otu}) {
                ...
                while (($sample,$abund)  = each %{ $table{$global_otu}}) {
                        ...
                        if ($abund >= 3) {
						##### removes global OTUs with zero, one, or two mapped reads only #####
